...
export default function testing() {
  ...
  const [viewType, setViewType] = useState(''); // New state for view type
...
//
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.length > 0 && data[0].CREATE_DATE) {
          setCreationDate(data[0].CREATE_DATE);
        }
  ...
  const renderViewB = () => (
    <>
      <Text style={styles.header}>Medium View</Text>
      <View style={styles.infoContainer}>
        <Text style={styles.infoTitle}>Medium Name: {medium.NAME}</Text>
        <Text style={styles.infoTitle}>Medium ID: {medium.ID}</Text>
        <Text style={styles.infoTitle}>Description: {medium.DESCRIPTION}</Text>
      </View>
    </>
  );
      ...
      <KeyboardAvoidingView style={styles.flex1} behavior={Platform.OS === "ios" ? "padding" : "height"} keyboardVerticalOffset={Platform.OS === "ios" ? 0 : -150}>
        <ScrollView contentContainerStyle={styles.scrollViewContainer}>
          <View style={styles.centeredContent}>
            ...
            <Text style={styles.title}>TEST ! Existing Tracked Regular Item</Text>
            ...
            {viewType === 'ITEM' ? renderViewA() : viewType === 'MEDIUM' ? renderViewB() : null}
